  ...
  return (
    <div className="page max-w-300 mx-auto">
      <section className="pt-10">
        ...
        <p
          className="mt-6 text-[17px] max-w-[52ch] leading-[1.55]"
          style={{ color: "var(--fg-dim)" }}
        >
          Modern applications focused on usability, performance, and scalable
          system design.{" "}
        </p>
        <div className="flex items-end justify-between gap-6 mt-14 mb-6 flex-wrap">
          <div className="count">
            {PROJECTS.length} projects · sorted recent
          </div>
        ...
        {view === "list" ? (
          <div className="border-t border-(--line) flex flex-col">
            {PROJECTS.map((p, i) => (
              <ProjectsRow key={p.id} project={p} idx={i} />
            ))}
          </div>
        ) : (
          <div className="Projects-grid grid grid-cols-2 gap-5 mt-6 max-[720px]:grid-cols-1">
            {PROJECTS.map((p, i) => {
              ...
              return (
                <div
                  className="Projects-card cursor-pointer"
                  key={p.id}
                  onClick={() =>
                    p.github &&
                ...
                >
                  ...
                  <div className="body p-[20px_22px_24px] flex flex-col gap-2">
                    <div className="flex justify-between items-baseline">
                      <div className="title text-[20px] font-medium tracking-[-0.02em]">
                        {p.title}
                      </div>
                    </div>
                    <div className="text-[13px] text-(--fg-dim) leading-normal">
                      {p.desc}
                    </div>
