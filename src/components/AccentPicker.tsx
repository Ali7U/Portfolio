...
export default function AccentPicker({ accentId, setAccentId, theme }: Props) {
  ...
  const triggerColor = (ACCENT_OPTIONS.find((o) => o.id === accentId) ??
    ACCENT_OPTIONS[0])[t];
  ...
  return (
    <div
      className="accent-picker-wrap relative pointer-events-auto sm:p-5"
      ref={wrapRef}
    >
      ...
      <button
        className="accent-mobile-trigger"
        onClick={() => setOpen((o) => !o)}
      ...
      {open && (
        <div className="accent-dropdown">
          {ACCENT_OPTIONS.map((opt) => (
            <button
              key={opt.id}
              className={`accent-dropdown-item ${accentId === opt.id ? "active" : ""}`}
              onClick={() => {
                setAccentId(opt.id);
                setOpen(false);
              }}
